    ...
    <b-card class="card-top-dash">
      <div class="d-flex justify-content-end">
        <b-button
          pill
          variant="dark"
          href="/brand-master-store-concept-edit"
          class="mx-1 mb-3"
          >New</b-button
        >
      </div>
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'event'">
              <a-button
                v-if="record.event === 'Yes'"
                class="btn-status-on"
                shape="round"
                >{{ record.event }}</a-button
              >
            ...
            <template v-if="column.key === 'shift'">
              <a-button
                v-if="record.shift === 'Yes'"
                class="btn-status-on"
                shape="round"
                >{{ record.shift }}</a-button
              >
              <a-button v-else class="btn-status-re" shape="round">{{
                record.shift
              }}</a-button>
            ...
            <template v-if="column.key === 'gender'">
              ...
              >
                <template #icon
                  ><edit-outlined two-tone-color="#52c41a" /></template>
              </a-button>
            </template>
             <template v-if="column.key === 'street'">
              <a-button
                type="text"
                shape="circle"
                href="/brand-master-store-concept-view"
                class="bt-tabs"
              >
                <template #icon
                  ><search-outlined two-tone-color="#52c41a" /></template>
              </a-button>
            </template>
          </template>
        </a-table>
...
const columns: TableColumnsType = [
  {
    ...
    sorter: (a: TableDataType, b: TableDataType) =>
      a.refNum.split('-').reverse().join('') !==
      b.refNum.split('-').reverse().join('')
        ? a.refNum.split('-').reverse().join('') <
          b.refNum.split('-').reverse().join('')
          ? -1
          : 1
  ...
  },
  {
    ...
    dataIndex: 'pic',
    key: 'pic',
    sorter: (a: TableDataType, b: TableDataType) => a.pic.length - b.pic.length
    // sorter: (a: TableDataType, b: TableDataType) =>
    //   a.companyName !== b.companyName
    //     ? a.companyName < b.companyName
    //       ? -1
...
const data = ref([
  {
    key: '1',
    refNum: 'Whatsapp',
    refDate: 'RIORSWHOLESALESTORE',
    companyName: 'RIORS WA STORE',
    number: '1345789',
    email: '@riors.id',
    pic: 'it01',
    shift: 'No',
  ...
  },
  {
    ...
    refNum: 'Tokopedia',
    refDate: 'RIORSWHOLESALESTORE',
    companyName: 'RIORS ONLINE STORE',
    number: '1345789',
    email: '@riors.id',
    pic: 'it01',
    shift: 'No',
